...
/*   Updated: 2019/04/06 23:07:33 by glormell         ###   ########.fr       */
/*                                                                            */
/* ************************************************************************** */
...
t_point3		rotate_x(t_point3 p, float angle)
{
	...
	n = point3(p.x, p.y * cos(angle) + p.z * sin(angle),
...
t_point3		rotate_y(t_point3 p, float angle)
{
	...
	n = point3(p.x * cos(angle) + p.z * sin(angle), p.y,
...
t_point3		rotate_z(t_point3 p, float angle)
{
	...
	n = point3(p.x * cos(angle) - p.y * sin(angle),
...
t_point3		rotate(t_fdf *fdf, t_point3 p)
{
	...
	px = rotate_x(p, -fdf->cvs.r.x);
	py = rotate_y(px, fdf->cvs.r.y);
	pz = rotate_z(py, fdf->cvs.r.z);
	return (pz);
}
